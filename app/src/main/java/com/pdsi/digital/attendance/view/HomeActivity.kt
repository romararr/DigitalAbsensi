...
import com.pdsi.digital.attendance.presenter.HomePresenter
import kotlinx.android.synthetic.main.confirm_attend_layout.view.*
...
//        liveLocation()
    ...
    private fun initActions() {
        masuk_button.onClick { confirmDialog() }
        pulang_button.onClick { confirmDialog(false) }
        istirahat_button.onClick { startActivity<AbsensiActivity>() }
...
//    }
    ...
    override fun onGetAbsenCompleteApi(q: MUser) {
        ...
        if (!data.time_come.isNullOrBlank()) come = data.time_come
        if (!data.time_return.isNullOrBlank()) out = data.time_return
...
//        toast("Location Refreshed")
    ...
    private fun confirmDialog(come: Boolean? = true) {
        val viewGroup = findViewById<ViewGroup>(android.R.id.content)
        val dialogView: View =
            LayoutInflater.from(this).inflate(R.layout.confirm_attend_layout, viewGroup, false)
        val builder: AlertDialog.Builder = AlertDialog.Builder(this)
        builder.setView(dialogView)

        val alertDialog: AlertDialog = builder.create()

        if (come != true) dialogView.tv_confirm.text = "Yakin Pulang Sekarang?"
        ...
        dialogView.confirm_btn.setOnClickListener {
            if (come == true) absenCome()
            else absenReturn()
            alertDialog.dismiss()
        }
    } // Note Out Office Attend
